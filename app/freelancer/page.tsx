...
export default function FreelancerDashboard() {
      ...
      if (profileData && currentJob) {
        ...
        if (adminProfile) {
          await createNotification(
            ...
            `${profileData.full_name} subió el entregable para "${currentJob.title}"`,
            jobId,
            'job'
          );
        }
  ...
  const proposalsByStatus = [
    ...
    { name: 'Aprobadas', value: stats.approvedProposals, color: STATUS_COLORS.aprobada },
    { name: 'Rechazadas', value: stats.rejectedProposals, color: STATUS_COLORS.rechazada },
    { name: 'Anuladas', value: proposals.filter(p => p.status === 'anulada').length, color: STATUS_COLORS.anulada },
  ].filter(item => item.value > 0);

  const earningsByCategoryMap = jobs.filter(j => j.status === 'completado').reduce((acc, job) => {
    ...
    <div className="min-h-screen bg-brand-crema flex flex-col">
      ...
      <main className="flex-grow max-w-5xl mx-auto w-full px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-brand-gris mt-1">{profile?.email} · Gestiona tus proyectos, postulaciones y perfil</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-brand-borde shadow-sm overflow-hidden mb-6">
          <div className="flex border-b border-brand-borde overflow-x-auto">
            <button onClick={() => setActiveTab('assigned')} className={`flex-1 px-6 py-4 font-semibold text-sm transition-colors whitespace-nowrap ${activeTab === 'assigned' ? 'text-brand-negro border-b-2 border-brand-rojo bg-brand-crema/30' : 'text-brand-gris hover:text-brand-negro'}`}>
              <Briefcase size={16} className="inline mr-2" /> Trabajos ({jobs.length})
            </button>
            <button onClick={() => setActiveTab('proposals')} className={`flex-1 px-6 py-4 font-semibold text-sm transition-colors whitespace-nowrap ${activeTab === 'proposals' ? 'text-brand-negro border-b-2 border-brand-rojo bg-brand-crema/30' : 'text-brand-gris hover:text-brand-negro'}`}>
              <FileText size={16} className="inline mr-2" /> Postulaciones ({proposals.length})
            </button>
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  <div className="bg-brand-crema/50 rounded-lg p-4 border border-brand-borde">
                    <div className="flex items-center gap-3">
                      <div className="bg-blue-100 p-2 rounded-lg"><Briefcase className="h-5 w-5 text-blue-600" /></div>
                      <div><p className="text-xs text-brand-gris font-semibold">En Progreso</p><p className="text-xl font-extrabold text-brand-negro">{stats.active}</p></div>
                    </div>
                  </div>
                  <div className="bg-brand-crema/50 rounded-lg p-4 border border-brand-borde">
                    <div className="flex items-center gap-3">
                      <div className="bg-yellow-100 p-2 rounded-lg"><Clock className="h-5 w-5 text-yellow-600" /></div>
                      <div><p className="text-xs text-brand-gris font-semibold">En Revisión</p><p className="text-xl font-extrabold text-brand-negro">{stats.inReview}</p></div>
                    </div>
                  </div>
                  <div className="bg-brand-crema/50 rounded-lg p-4 border border-brand-borde">
                    <div className="flex items-center gap-3">
                      <div className="bg-green-100 p-2 rounded-lg"><Award className="h-5 w-5 text-green-600" /></div>
                      <div><p className="text-xs text-brand-gris font-semibold">Completados</p><p className="text-xl font-extrabold text-brand-negro">{stats.completed}</p></div>
                    </div>
                  </div>
                  <div className="bg-brand-crema/50 rounded-lg p-4 border border-brand-borde">
                    <div className="flex items-center gap-3">
                      <div className="bg-brand-rojo/10 p-2 rounded-lg"><DollarSign className="h-5 w-5 text-brand-rojo" /></div>
                      <div><p className="text-xs text-brand-gris font-semibold">Ingresos</p><p className="text-xl font-extrabold text-brand-negro">${stats.totalEarned.toLocaleString()}</p></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-6 flex flex-col sm:flex-row gap-3">
                  <div className="flex-1 relative">
                    ...
                    <input type="text" placeholder="Buscar por título, categoría o descripción..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="tm-input pl-10" />
                  </div>
                ...
                {filteredJobs.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-brand-gris mt-2 max-w-md mx-auto">Explora el tablero de trabajos disponibles y postúlate a los proyectos que te interesen.</p>
                    <button onClick={() => router.push('/')} className="tm-btn-rojo mt-6 inline-flex items-center gap-2">Ver Trabajos Disponibles</button>
                  </div>
                ) : (
                      ...
                      <div key={job.id} className="bg-brand-crema/30 rounded-xl border border-brand-borde overflow-hidden hover:shadow-md transition-shadow">
                        <div className="p-6">
                          <div className="flex justify-between items-start mb-4 flex-wrap gap-3">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-2">
                                <span className="bg-brand-crema text-brand-vino font-semibold text-xs px-3 py-1 rounded-full">{job.category}</span>
                            ...
                            <div className="text-right">
                              ...
                              <p className="text-2xl font-extrabold text-brand-negro">${Number(job.budget).toLocaleString()}</p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between pt-4 border-t border-brand-borde flex-wrap gap-3">
                            <div className="text-xs text-brand-gris">Publicado: {new Date(job.created_at).toLocaleDateString('es-ES', { day: '2-digit', month: 'long', year: 'numeric' })}</div>
                            ...
                            <div className="flex items-center gap-3 flex-wrap">
                              {job.status === 'en_progreso' && (
                                <label className="tm-btn-rojo flex items-center gap-2 cursor-pointer">
                                  ...
                                  <input type="file" className="hidden" onChange={(e) => { if (e.target.files && e.target.files[0]) { handleUploadDeliverable(job.id, e.target.files[0]); } }} accept=".pdf,.zip,.rar,.doc,.docx,.jpg,.png" />
                                </label>
                              )}

                              {job.status === 'en_revision' && (
                                <div className="flex items-center gap-2 text-yellow-700 bg-yellow-50 px-4 py-2 rounded-lg">
                                  <Clock size={16} /><span className="text-sm font-semibold">Esperando revisión</span>
                                </div>
                              )}

                              {job.status === 'completado' && (
                                <div className="flex items-center gap-2 text-green-700 bg-green-50 px-4 py-2 rounded-lg">
                                  <CheckCircle size={16} /><span className="text-sm font-semibold">Completado</span>
                                </div>
                              )}
                              ...
                              <button onClick={() => setChatJob(job)} className="tm-btn-outline flex items-center gap-1 text-xs relative">
                                ...
                                {job.unread_messages && job.unread_messages > 0 && (
                                  <span className="absolute -top-2 -right-2 bg-brand-rojo text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center animate-pulse">{job.unread_messages}</span>
                                )}
                              </button>
                            </div>
                          </div>
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className="bg-brand-crema/50 rounded-lg p-4 border border-brand-borde">
                    <div className="flex items-center gap-3">
                      <div className="bg-yellow-100 p-2 rounded-lg"><Clock className="h-5 w-5 text-yellow-600" /></div>
                      <div><p className="text-xs text-brand-gris font-semibold">Pendientes</p><p className="text-xl font-extrabold text-brand-negro">{stats.pendingProposals}</p></div>
                    </div>
                  </div>
                  <div className="bg-brand-crema/50 rounded-lg p-4 border border-brand-borde">
                    <div className="flex items-center gap-3">
                      <div className="bg-green-100 p-2 rounded-lg"><CheckCircle className="h-5 w-5 text-green-600" /></div>
                      <div><p className="text-xs text-brand-gris font-semibold">Aprobadas</p><p className="text-xl font-extrabold text-brand-negro">{stats.approvedProposals}</p></div>
                    </div>
                  </div>
                  <div className="bg-brand-crema/50 rounded-lg p-4 border border-brand-borde">
                    <div className="flex items-center gap-3">
                      <div className="bg-brand-rojo/10 p-2 rounded-lg"><TrendingUp className="h-5 w-5 text-brand-rojo" /></div>
                      <div><p className="text-xs text-brand-gris font-semibold">Total Postulaciones</p><p className="text-xl font-extrabold text-brand-negro">{proposals.length}</p></div>
                    </div>
                  </div>
                </div>
                ...
                {proposals.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-brand-gris mt-2 max-w-md mx-auto">Explora el tablero de trabajos y postúlate a los proyectos que te interesen.</p>
                    <button onClick={() => router.push('/')} className="tm-btn-rojo mt-6 inline-flex items-center gap-2">Ver Trabajos Disponibles</button>
                  </div>
                ) : (
                      ...
                      <div key={proposal.id} className="bg-brand-crema/30 rounded-lg border border-brand-borde p-5 hover:shadow-md transition-shadow">
                        ...
                        <div className="flex justify-between items-center flex-wrap gap-3 text-sm">
                          <div className="flex gap-4">
                            <span className="font-semibold text-brand-negro">Ofertado: ${Number(proposal.proposed_budget).toLocaleString()}</span>
                            <span className="text-brand-gris">Presupuesto cliente: ${Number(proposal.jobs.budget).toLocaleString()}</span>
                          </div>
                          <span className="text-xs text-brand-gris">{new Date(proposal.created_at).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                        </div>
                        {proposal.status === 'rechazada' && (
                          <div className="mt-3 flex items-center gap-2 text-brand-rojo text-sm">
                            <XCircle size={14} /><span>Esta propuesta no fue seleccionada</span>
                          </div>
                        )}
                      </div>
                    ))}
            ...
            {activeTab === 'stats' && (
              <div className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-gradient-to-br from-brand-negro to-gray-800 rounded-xl p-5 text-white">
                    <div className="flex items-center gap-3 mb-2"><DollarSign className="h-5 w-5 text-green-400" /><p className="text-xs text-gray-300 uppercase font-semibold">Total Ganado</p></div>
                    <p className="text-3xl font-extrabold">${stats.totalEarned.toLocaleString()}</p>
                  </div>
                  <div className="bg-gradient-to-br from-brand-negro to-gray-800 rounded-xl p-5 text-white">
                    <div className="flex items-center gap-3 mb-2"><TrendingUp className="h-5 w-5 text-blue-400" /><p className="text-xs text-gray-300 uppercase font-semibold">Tasa de Aprobación</p></div>
                    <p className="text-3xl font-extrabold">{approvalRate}%</p>
                    <p className="text-xs text-gray-400 mt-1">{stats.approvedProposals} de {proposals.length} propuestas</p>
                  </div>
                  <div className="bg-gradient-to-br from-brand-negro to-gray-800 rounded-xl p-5 text-white">
                    <div className="flex items-center gap-3 mb-2"><Award className="h-5 w-5 text-yellow-400" /><p className="text-xs text-gray-300 uppercase font-semibold">Completados</p></div>
                    <p className="text-3xl font-extrabold">{stats.completed}</p>
                    <p className="text-xs text-gray-400 mt-1">trabajos finalizados</p>
                  </div>
                  <div className="bg-gradient-to-br from-brand-negro to-gray-800 rounded-xl p-5 text-white">
                    <div className="flex items-center gap-3 mb-2"><Briefcase className="h-5 w-5 text-brand-rojo" /><p className="text-xs text-gray-300 uppercase font-semibold">Promedio por Trabajo</p></div>
                    <p className="text-3xl font-extrabold">${avgBudget.toLocaleString()}</p>
                    <p className="text-xs text-gray-400 mt-1">presupuesto promedio</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="bg-brand-crema/30 rounded-xl p-6 border border-brand-borde">
                    ...
                    {chartEarningsByMonth.length === 0 ? (
                      <div className="h-64 flex items-center justify-center text-brand-gris text-sm">Aún no hay trabajos completados</div>
                    ) : (
                      <div className="h-64 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={chartEarningsByMonth}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                            <XAxis dataKey="name" stroke="#6B7280" fontSize={12} />
                            <YAxis stroke="#6B7280" fontSize={12} />
                            <Tooltip formatter={(value) => `$${Number(value || 0).toLocaleString()}`} />
                            <Bar dataKey="ingresos" fill="#D9374A" radius={[4, 4, 0, 0]} />
                          </BarChart>
                        </ResponsiveContainer>
                      </div>
                    )}
                  ...
                  <div className="bg-brand-crema/30 rounded-xl p-6 border border-brand-borde">
                    <h3 className="text-lg font-bold text-brand-negro mb-4 flex items-center gap-2"><PieChartIcon size={20} className="text-brand-vino" /> Estado de Propuestas</h3>
                    {proposalsByStatus.length === 0 ? (
                      <div className="h-64 flex items-center justify-center text-brand-gris text-sm">Aún no has enviado propuestas</div>
                    ) : (
                      <>
                        <div className="h-64 w-full">
                          <ResponsiveContainer width="100%" height="100%">
                  ...
                  <div className="bg-brand-crema/30 rounded-xl p-6 border border-brand-borde lg:col-span-2">
                    ...
                    {chartEarningsByCategory.length === 0 ? (
                      ...
                      <div className="h-64 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={chartEarningsByCategory} layout="vertical">
                            <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                            <XAxis type="number" stroke="#6B7280" fontSize={12} />
                            <YAxis type="category" dataKey="name" stroke="#6B7280" fontSize={12} width={120} />
                            <Tooltip formatter={(value) => `$${Number(value || 0).toLocaleString()}`} />
                            <Bar dataKey="value" fill="#6E1423" radius={[0, 4, 4, 0]} />
                          </BarChart>
                        </ResponsiveContainer>
                      </div>
                    )}
